GobiNet network driver 2017-02-24
...
This Release (GobiNet network driver 2017-02-24)
a. Fixed setting the correct MUX Id for IPV6 packets.
b. Added enableDhcp module param to enable sending DHCP packets for IP configuration.

Prior Release (GobiNet network driver 2017-02-09)
a. Send QMI_WDS_SET_CLIENT_IP_FAMILY_MSG after sending QMI_WDS_BIND_MUX_DATA_PORT REQ.

Prior Release (GobiNet network driver 2017-01-24)
a. Added IPv6 support.

Prior Release (GobiNet network driver 2016-10-14)
...
>echo 1 > /sys/testgobi/gobi0, this will initiate a data call.
 ...
 ifconfig  usb0 inet6 add <ipv6 address>
...
>echo 1 > /sys/testgobi/gobi1, this will initiate a data call.
 ...
 ifconfig  usb0:0 inet6 add <ipv6 address>
...
>echo 1 > /sys/testgobi/gobi2, this will initiate a data call.
 ...
 ifconfig  usb0:1 inet6 add <ipv6 address>
...
enalbeDhcp Feature:
-------------------
Installing with Dhcp Enabled : > insmod GobiNet.ko enableDhcp=1
LIMIATION: The limitation of this feature is MUXing feature cannot be used, the IP address 
can only be assigned only to the primary adapter.
